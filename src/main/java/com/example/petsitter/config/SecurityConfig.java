    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        http
                .authorizeHttpRequests((auth)-> auth
                        .requestMatchers("/","/login","/loginProc","/join","/joinProc","/images/**","/css/**", "/idcheck","/api/**", "/api/kakao/**", "/petsitter/**").permitAll()
                        .requestMatchers("/admin").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.DELETE,"/my/**", "/pet/**","/pet/view/**", "/pet/delete/**", "/pet/update/**").hasAnyRole("ADMIN","USER","MANAGER") // ** 와일드카드
                        .requestMatchers("/petsitter/sitterRole/**").hasAnyRole("MANAGER","ADMIN")
